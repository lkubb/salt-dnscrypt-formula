...
We will see a quick setup to configure the ``dnscrypt`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``dnscrypt`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``dnscrypt`` formulas under ``/srv/salt/dnscrypt/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/dnscrypt/parameters/dns:domain/
    ln -s dns:domain /srv/salt/dnscrypt/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/dnscrypt/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/dnscrypt/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``dnscrypt`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``dnscrypt`` formulas under ``/srv/salt/dnscrypt/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/dnscrypt/parameters/roles
...
We create a configuration for the role ``dnscrypt/server`` in ``/srv/salt/dnscrypt/parameters/roles/dnscrypt/server.yaml``:
...
We create another configuration for the role ``dnscrypt/client`` in ``/srv/salt/dnscrypt/parameters/roles/dnscrypt/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles dnscrypt/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles dnscrypt/client
...
.. code-block:: console
    ...
    mv /srv/salt/dnscrypt/parameters/dns:domain/  '/srv/salt/dnscrypt/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-dnscrypt-as-json:
      test.nop:
        - name: {{ dnscrypt | json }}
...
Here is an example based on `template-formula/dnscrypt/config/file.sls`_:
    ...
    dnscrypt-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='dnscrypt-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ dnscrypt.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``dnscrypt`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ dnscrypt['winner'] }}
...
.. _template-formula/dnscrypt/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/dnscrypt/config/file.sls
